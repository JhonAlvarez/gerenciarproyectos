	...
	<div align="center">
    	<table width="90%">
          <tr>
			...
			<?php
			$cod_sector=$_POST['cod_sector'];
			$sector=$_POST['sector'];


		$consultaInsertarCod_sector=mysql_query("SELECT * FROM sectores WHERE cod_sector='$cod_sector' ");
		if($row=mysql_fetch_array($consultaInsertarCod_sector)){
			echo mensajes('Este sector ya existe','rojo');
		}else{
			echo mensajes('Este sector fue agragado exitosamente','verde');
			mysql_query("INSERT INTO sectores (cod_sector, sector) VALUES ('$cod_sector', '$sector')");
		...
		<table border="1">
		  <tr>
			...
			<?php
				$consultasector=mysql_query("SELECT * FROM sectores ORDER BY cod_sector DESC");
