    	...
    	<table width="90%">
          <tr>
            <td>
                ...
                <div align="right">
			<form method="POST" action="crear_meta7.php">
				<?php
				...
				$consultaprograma=mysql_query("SELECT * FROM plandedesarrolloprograma WHERE cod_eje=$cod_eje and cod_plan=$cod_plan and cod_estrategia=$cod_estrategia and cod_programa=$cod_programa");
				...
				echo "<tr>";
				echo "<td>Codigo Subprograma: </td>";
				echo "<td><input type='text' name='cod_subprograma' readonly value='".$cod_subprograma."'></td>";
				$consultasubprograma=mysql_query("SELECT * FROM plandedesarrollosubprograma WHERE cod_eje=$cod_eje and cod_plan=$cod_plan and cod_estrategia=$cod_estrategia and cod_programa=$cod_programa and cod_subprograma=$cod_subprograma");
					while($filasubprograma=mysql_fetch_array($consultasubprograma)){
						echo '<td>Subprograma: </td><td><input type="text" name="subprograma" readonly value="'.$filasubprograma['subprograma'].'"></td>';
					}
				...
				echo "</tr>";
				echo "<tr>";
				$consultameta=mysql_query("SELECT MAX(cod_meta) AS ultimoMeta FROM plandedesarrollometa");
					while($filameta=mysql_fetch_array($consultameta)){
						$nuevoCod_meta=$filameta['ultimoMeta']+1;
						echo '<td>Codigo Meta: </td><td><input type="text" name="cod_meta" readonly value="'.$nuevoCod_meta.'"></td>';
					}


					?>
				<td>
				<strong>Escriba el Nombre de la Meta: </strong>
				</td>
				<td>
